...
ssh "${REMOTE_HOST}" "cd ${REMOTE_ROOT} && tmux new-session -d -s adh-phase2 'cd ${REMOTE_ROOT} && . .venv/bin/activate && exec .venv/bin/uvicorn server.main:app --host 0.0.0.0 --port 9000 > server/logs/runtime.log 2>&1'"
