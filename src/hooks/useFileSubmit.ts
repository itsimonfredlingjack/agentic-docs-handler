...
async function reconcileProcessResponse(
  ...
  applyMoveFinalized: (payload: {
    ...
    move_status: "not_requested" | "planned" | "awaiting_confirmation" | "auto_pending_client" | "moved" | "move_failed" | "undone";
  }) => void,
): Promise<void> {
...
export function useFileSubmit() {
  ...
  useEffect(() => {
    void cleanupStagedUploads(24).catch((error) => {
      console.error("useFileSubmit.cleanup_staged_uploads.failed", error);
    });
  }, []);
    ...
    async (files: File[]) => {
      ...
      const jobs = await Promise.all(
        files.map(async (file, index) => {
          ...
          return { file, requestId, sourcePath, stagingError };
        }),
      );
      ...
      try {
        ...
        console.error("useFileSubmit.refresh.failed", error);
      }
    },
    [clientId, queueUploads, rememberUpload, clearRememberedUpload, upsertDocument, markJobFailed, applyMoveFinalized, bootstrap],
  );
  ...
  return { submitFiles, openFilePicker, handleInputChange, fileInputRef, tauriDragOver };
}
